...
# Internal imports
from .mitigation.action import mitigation_action_model, UpdateActionStatusRequest, User
from .mitigation.rules import playbook_creator
from .auth.jwt import create_access_token
from .auth.oauth import get_current_user, oauth2_scheme
...
app = FastAPI(title="RTR Service API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# In-memory storage for mitigation actions (TODO: move to database)
mitigation_actions: Dict[str, dict] = {}


@app.get("/")
def root():
    return {"message": "Welcome to RTR Service"}


@app.post('/register')
def create_user(request: User):
    # Check if user already exists in MongoDB
    users_collection = db_connection.users_collection
...
@app.get("/action_by_id/{intent_id}")
...
@app.post("/update_action_status", status_code=status.HTTP_200_OK)
